 ...
 * @return {Object}
 */
function assignAttribute(element, name, value, prefix) {
  const attr = prefix ? `${prefix}-${name}` : name;

  if (isObject(value)) {
    // eslint-disable-next-line no-use-before-define
    return assignAttributes(element, value, attr);
  }
 ...
 * @return {Object}
 */
export function assignAttributes(element, attrs, prefix) {
  Object
    .keys(attrs)
    .forEach((name) => assignAttribute(element, name, attrs[name], prefix));
